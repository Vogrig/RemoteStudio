...
RemoteStudio is a web application that can be controlled by multiple users at the same time. Web Sockets are used to exchange data between client and server. The server side is responsible of the OSC communication, to avoid UPD connection on the client web page.
